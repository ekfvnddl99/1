            ...
            public void onClick(View view) {
                ...
                del.setNegativeButton("예", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        Present.list.remove(idx);

                        JSONObject obj=new JSONObject();
                        JSONArray arr=new JSONArray();
                        try {
                            for(int j=0; j<Present.list.size(); j++){
                                JSONObject tmp=new JSONObject();
                                tmp.put("name",Present.list.get(j).getName());
                                tmp.put("number",Present.list.get(j).getNumber());
                                tmp.put("friendly",Present.list.get(j).getFriendly());
                                arr.put(tmp);
                            }
                            obj.put("Phonebook", arr);
                        } catch (JSONException e) {
